...
validate_xkcd <- function(x){
  ...
  required_fields <- c("month", "num", "link", "year", "news", "safe_title", "transcript", "alt", "img", "title", "day"
                       )
  ...
  char_fields <- c("month", "link", "year", "news", "safe_title", "transcript", "alt", "img", "title", "day"
  )
  ...
  if(!(is.numeric(x[["num"]]) && length(x[["num"]]) == 1)){
    stop("The ", f, "field in an xkcd object must be a numeric vector of length 1")
  }
